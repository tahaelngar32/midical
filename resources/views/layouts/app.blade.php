...
<body class="bg-slate-50 text-slate-800" data-active-view="@yield('active_view', 'dashboard')">
  ...
  <div class="dashboard-shell">
    ...
    <aside id="sidebar" class="sidebar-panel fixed left-0 top-0 z-40 h-screen w-[250px] -translate-x-full lg:translate-x-0">
      <div class="flex items-center gap-3 border-b border-slate-100 px-5 py-6">
        <div class="grid h-9 w-9 place-items-center rounded-xl bg-slate-700 text-sm font-bold text-white">T</div>
    ...
    <main class="main-panel min-h-screen lg:ml-[250px]">
      <div class="content-wrap px-4 py-4 sm:px-5 lg:px-6 lg:py-6">
        ...
        <header class="topbar-card mb-3 flex flex-wrap items-center gap-3">
          <button onclick="openSidebar()" class="grid h-11 w-11 place-items-center rounded-2xl border border-slate-200 text-slate-700 lg:hidden">
  ...
  <div id="toast" class="pointer-events-none fixed bottom-5 right-5 translate-y-10 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white opacity-0 shadow-lg transition-all duration-300">Saved</div>
